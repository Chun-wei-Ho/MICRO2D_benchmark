...
def load_train_test(filename):
    ...
    structs_reshaped = structs.reshape(10000, 64, 4, 64, 4)

    # Take the mean along the 2x2 blocks to downsample
    structs_downsampled = np.mean(structs_reshaped, axis=(2, 4))
    print("Downsampled structs shape:", structs_downsampled.shape)
    
    print("Pre-processing with TPS and PCA")
    structs = preprocess(structs_downsampled)
